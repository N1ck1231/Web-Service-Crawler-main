...
public interface SdksMapper extends BaseMapper<Sdks> {
    @Select("SELECT COUNT(*) AS total,categories FROM (  SELECT CASE WHEN INSTR(categories,'#')>0 THEN LEFT(categories,INSTR(categories,'#')-1)  " +
            "  ELSE categories END categories  FROM sdks) temp  GROUP BY categories  ORDER BY COUNT(*) DESC")
    ...
    @Select("SELECT COUNT(*) AS total,SUBSTRING_INDEX(SUBSTRING_INDEX(tag,'###',b.help_topic_id + 1),'###' ,- 1) AS tag2\n" +
            "    FROM  (SELECT  CASE WHEN INSTR(categories,'#')>0 THEN SUBSTR(categories,INSTR(categories,'#')+3) ELSE NULL  END tag ,categories FROM sdks) AS temp\n" +
