<template>
  <div class="app-container">
    ...
    <el-table class='mashupsTable' :data="list" border stripe :cell-style="{textAlign:'center'}">
      ...
      <el-table-column prop="sdksName" label="sdksName" width='105'>
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
            type='text' @click='sortSearch("sdksName")'>
            sdksName
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="relatedApis" label="relatedApis" width='110' show-overflow-tooltip >
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
      ...
      <el-table-column prop="categories" label="categories" width='105' >
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
      ...
      <el-table-column prop="addedDate" label="addedDate" width='105'>
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
      ...
      <el-table-column prop="sdkProvider" label="sdkProvider" width='115'>
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
            type='text' @click='sortSearch("sdkProvider")'>
            sdkProvider
          </el-button>
        </template>
      </el-table-column>
      <el-table-column prop="followersNumber" label="followersNumber" width='145' >
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
      ...
      <el-table-column prop="followers" label="followers" width='100' >
        <template slot="header">
          <el-button
            class="el-icon-caret-bottom btn-sort"
...
<script>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        sdks.pageRecords(this.currentPage,this.currentSize,this.sortName).then(response => {
      ...
      //导出数据
      exportData(){
        window.location.href = this.BASE_API + "/sdks/export"
      },
      searchLikeName(){
        sdks.searchLikeName(this.selectName,this.searchValue,this.currentPage,this.currentSize,this.sortName).then(response=>{
          this.total = response.data.slice(-1)[0].total
          this.list  = response.data.slice(0,-1)
